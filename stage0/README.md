...
Stage0bFilterJob
  输入原始 CSV
  -> DistributedCache 加载 Stage0a 的 vid 集合
  -> 解析完整记录
  -> 命中 vid 集合则写出 RecordWritable
```

如果使用 Bloom Filter，必须保证业务上不产生假阴性：出现次数 `>= 2` 的 vid 不能被过滤掉。Bloom Filter 参数如果要配置化，需要先补到 `companion-conf.xml`、`CompanionConf` 和 `docs/architecture.md`。

## 必须保持的契约

- 输出值固定为 `RecordWritable`，字段顺序为 `(vid, loc, ts_norm)`。
- `RecordWritable` 字节布局固定为 12 字节，也就是 3 个 `int`。
- 输出使用 `SequenceFile`，并启用 Snappy block 压缩。
...
- 不要用全量 `HashSet<Integer>` 保存 2300 万车辆，优先使用 Bloom Filter 或更紧凑的集合格式。
